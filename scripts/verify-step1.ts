...
const expectedStores = ["Java House Kigali Heights", "Karame Bay Market"];
...
async function verify() {
  ...
  if (stores.length!==2 || !expectedStores.every(name=>stores.some(store=>store.name===name))) throw new Error("Phase 1 stores are incorrect");
